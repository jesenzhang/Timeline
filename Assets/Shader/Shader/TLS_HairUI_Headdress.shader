...
Shader "TLStudio/Character/HairUIHeaddress" {
Properties {
		...
		Pass {  
			CGPROGRAM
				...
				fixed4 frag (v2f i) : SV_Target
				{
					...
					float CutAlpha = lerp (col.b,col.a,col.a);
					clip(CutAlpha - _Cutoff);
					...
					fixed4 hairColorTemp = lerp(_ColorB,_ColorA,col.r)*col.g;
					fixed4 hairColor = lerp(hairColorTemp,col,col.a);
					float gloss = lerp(pow(col.g,5),0.8,col.a);
					fixed4 final = diffLight*hairColor+_Reflection_var*gloss+rim*rim*_RimColor;
					return final;
				}
			ENDCG
		}

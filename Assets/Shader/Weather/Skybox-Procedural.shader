Shader "Weather/Skybox-Procedural" {
Properties {
	...
	_MinExposure("Min Exposure",Range(0,0.8)) = 0.1
	_DayIntensity("_DayIntensity",Range(0,1)) = 1
}
...
//				for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
			...
			float dayIntensity = _MinExposure + clamp(_DayIntensity,0,1 - _MinExposure);

			OUT.groundColor	= _Exposure * dayIntensity * (cIn + COLOR_2_LINEAR(_GroundColor) * cOut);
			OUT.skyColor	= _Exposure * dayIntensity * (cIn * getRayleighPhase(_WorldSpaceLightPos0.xyz, -eyeRay));

		#if SKYBOX_SUNDISK != SKYBOX_SUNDISK_NONE
			OUT.sunColor	= _Exposure * dayIntensity * (cOut * _LightColor0.xyz);
		#endif

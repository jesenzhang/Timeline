...
inline UnityGI UnityGI_Base(UnityGIInput data, half occlusion, half3 normalWorld)
{
    ...
    o_gi.light.color *= data.atten;
        ...
        // cy engine
        #if defined(CYENGINE_LIGHTMAP_FADE)
            half4 bakedColorTexFade = UNITY_SAMPLE_TEX2D(unity_LightmapFade, data.lightmapUV.xy);
            bakedColorTex = lerp(bakedColorTex, bakedColorTexFade, cyengine_lightmap_lerp);
        #endif
        ...
        half3 bakedColor = DecodeLightmap(bakedColorTex);
            ...
            // cy engine
            #if defined(CYENGINE_LIGHTMAP_FADE)
                half4 bakedDirTexFade = UNITY_SAMPLE_TEX2D_SAMPLER (unity_LightmapIndFade, unity_LightmapFade, data.lightmapUV.xy);
                bakedDirTex = lerp(bakedDirTex, bakedDirTexFade, cyengine_lightmap_lerp);
            #endif
        ...
        // cy engine
        #if defined(CYENGINE_LIGHTMAP_FADE)
            fixed4 realtimeColorTexFade = UNITY_SAMPLE_TEX2D(unity_DynamicLightmapFade, data.lightmapUV.zw);
            realtimeColorTex = lerp(realtimeColorTex, realtimeColorTexFade, cyengine_lightmap_lerp);
        #endif
        ...
        half3 realtimeColor = DecodeRealtimeLightmap (realtimeColorTex);
            ...
            half4 realtimeDirTex = UNITY_SAMPLE_TEX2D_SAMPLER(unity_DynamicDirectionality, unity_DynamicLightmap, data.lightmapUV.zw);
             // cy engine
            #if defined(CYENGINE_LIGHTMAP_FADE)
                half4 realtimeDirTexFade = UNITY_SAMPLE_TEX2D_SAMPLER(unity_DynamicDirectionalityFade, unity_DynamicLightmapFade, data.lightmapUV.zw);
                realtimeDirTex = lerp(realtimeDirTex, realtimeDirTexFade, cyengine_lightmap_lerp);
            #endif
...
//#define CUBEMAPUSEMIP 1

#if defined(CUBEMAPUSEMIP)
UNITY_DECLARE_TEXCUBE(_GI_CubeMap);
#else
samplerCUBE _GI_CubeMap;
#endif
inline half3 UnityGI_IndirectSpecular(UnityGIInput data, half occlusion, Unity_GlossyEnvironmentData glossIn)
{
    half3 specular;
        ...
        //return originalReflUVW;
    #endif

    #ifdef _GLOSSYREFLECTIONS_OFF
        specular = unity_IndirectSpecColor.rgb;
    #else
        #if defined(_CUBEMAPENABLE_ON) && defined(CUBEMAPUSEMIP) 
            ...
            half4 rgbm = UNITY_SAMPLE_TEXCUBE_LOD(_GI_CubeMap, R, mip);
            half4 hdr = data.probeHDR[0];
            //half3 env0 =  DecodeHDR(rgbm, hdr);
            half3 env0 = rgbm.xyz;
...
inline UnityGI UnityGlobalIllumination (UnityGIInput data, half occlusion, half3 normalWorld, Unity_GlossyEnvironmentData glossIn)
{
    ...
    #if defined(_CUBEMAPENABLE_ON) && !defined(CUBEMAPUSEMIP) 
        half3 refDir = reflect(-data.worldViewDir, normalWorld);
        float3 refColor = texCUBE(_GI_CubeMap, refDir).rgb;
        o_gi.indirect.specular = (refColor * occlusion);
    #else

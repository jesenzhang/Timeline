...
//////////////////////////////////////////////////////
	...
	uniform half _GlowIntensityInner;
		...
		uniform half _GlowIntensityOuter;

		#if _MK_LENS
		uniform sampler2D _LensTex;
		uniform half _LensIntensity;
		#endif
	#endif
	...
	fixed4 frag( Output i ) : SV_TARGET
	{
		...
		#ifndef FULLSCREEN
			fixed3 gO = tex2D(_MKGlowTexOuter, UnityStereoScreenSpaceUVAdjust(i.uv1.xy, _MKGlowTexInner_ST)) * _GlowIntensityOuter;

			g = lerp(g,gO,0.5);

			#if _MK_LENS
				fixed3 lens = tex2D(_LensTex, UnityStereoScreenSpaceUVAdjust(i.uv1.xy, _MKGlowTexInner_ST)).rgb;
				g = lerp(g, (g+g*lens)*0.5, _LensIntensity);
			#endif
		#endif
